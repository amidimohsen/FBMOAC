...
|forward states $\mathbf{s}_t$ and backward states  $\mathbf{y}_t$ apply the same actions $\mathbf{a}_t$, but with a different ordering in time. Moreover, FB-MDPs includes forward transition probability determining the evolution of the forward state $s_{t+1} ~ P_f(\cdot | \mathbf{s}_t,\mathbf{a}_t)$  and backqard transition probability specifying the evolution of the backward state.|
...
using a policy $\pi(\cdot | \mathbf{s}_t)$, (ii) **backward evaluation**, in which the backward dynamics is evaluated in a time-reversed way 
...
to optimize the policy $\pi(\cdot | \mathbf{s}_t)$ based on the experiences obtained from both the forward and backward dynamics. <br>
