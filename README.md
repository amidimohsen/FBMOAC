...
|forward states **s**_t and backward states  **y**_t apply the same actions **a**_t, but with a different ordering in time. Moreover, FB-MDPs includes forward transition probability determining the evolution of the forward state and backqard transition probability specifying the evolution of the backward state.|
